...
export default function Row({ teamMemberInviteId }: RowProps) {
  ...
  const deleteTeamMemberInviteConfirmation = () => {
    dispatch(
      displayPrompt('Delete invite', 'Would you like to delete this team member invite?', [
        {
          label: 'Delete invite',
          style: 'danger',
          callback: async () => {
    ...
    const tempPropdownItemsForPendingInvite = [
      {
        ...
        icon: <ArrowPathIcon className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
      },
      {
        label: 'Delete',
        onClick: deleteTeamMemberInviteConfirmation,
        icon: <UserMinusIcon className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
      }
    ];
  ...
  return teamMemberInvite ? (
    <tr key={teamMemberInvite.id}>
      ...
      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
        {teamMemberInvite.accepted_at === null && moment(teamMemberInvite.expires_at).isBefore(moment()) && (
          <Badge value="Expired" textColour="text-gray-800" backgroundColour="bg-gray-100" />
        )}
        {teamMemberInvite.accepted_at === null && moment(teamMemberInvite.expires_at).isAfter(moment()) && (
          <Badge value="Pending" textColour="text-blue-800" backgroundColour="bg-blue-100" />
        )}
        {teamMemberInvite.accepted_at !== null ? (
          <Badge value="Accepted" textColour="text-green-800" backgroundColour="bg-green-100" />
        ) : null}
      </td>
      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
        {teamMemberInvite.created_at ? OutputDateTime(teamMemberInvite.created_at, null) : '-'}
      </td>
      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
        {teamMemberInvite.expires_at ? OutputDateTime(teamMemberInvite.expires_at, null) : '-'}
      </td>
