...
interface RowProps {
  inboxMember: IInboxMember;
}

export default function Row({ inboxMember }: RowProps) {
  const { inboxId } = useParams();
  const queryClient = useQueryClient();

  const removeTeamMemberInboxAccessMutation = useMutation(
    removeTeamMemberInboxAccessService,
    {
      onSuccess: () => {
        ...
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <div className="font-medium text-gray-900">
              {inboxMember.team_member.user.name}
            </div>
            <div className="text-gray-500">
              {inboxMember.team_member.user.email}
            </div>
          </div>
        </div>
      </td>
      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
        {inboxMember.access_level.name}
      </td>
