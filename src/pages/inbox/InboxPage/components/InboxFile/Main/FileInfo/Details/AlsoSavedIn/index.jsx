...
import { useInboxFile } from '../../../../../../../../features/inbox/inboxService';

function AlsoSavedIn() {
  const selectedInboxFileId = useSelector((state) => state.inbox.selectedInboxFileId);
  const { data: inboxFile } = useInboxFile(selectedInboxFileId);
  ...
  return inboxFile ? (
    <div>
      ...
      <ul className="mt-2 border-t border-b border-gray-200 divide-y divide-gray-200">
        {selectedInboxFileFullDetails.inbox_file.also_saved_in_files.map((alsoSavedInFile) => (
          <li key={alsoSavedInFile.id} className="py-3 flex justify-between items-center">
            <div className="flex items-center space-x-4">
              <div className="flex-shrink-0">
                <FileIcon iconName={inboxFile.inbox_file.file_format.icon_name} size={8} />
              </div>
