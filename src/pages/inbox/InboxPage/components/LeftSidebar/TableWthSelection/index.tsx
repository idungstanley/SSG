...
import { useGetInboxFiles, useMultipleArchiveOrUnArchive } from '../../../../../../features/inbox/inboxService';
...
export default function TableWithSelection() {
  ...
  const { selectedInboxTabKey, selectedInboxFileId, selectedInboxFileIndex } = useAppSelector((state) => state.inbox);
    ...
    () =>
      data?.pages.flatMap((page) =>
        page.data.inbox_files.map((i) =>
          inboxFiles.push({
            id: i.id,
            name: <FileIcon extensionKey={i.inbox_file_source.file_format.key} size={10} />,
  ...
  useLayoutEffect(() => {
    ...
    if (selectedFiles.length === inboxFiles.length && +selectedFiles.length + +inboxFiles.length > 0) {
  ...
  function toggleAll() {
    setSelectedFiles(checked || indeterminate ? [] : inboxFiles.map((i) => i.id));
  ...
  const handleChangeInbox = (e: React.ChangeEvent<HTMLInputElement>, fileId: string) => {
    setSelectedFiles(e.target.checked ? [...selectedFiles, fileId] : selectedFiles.filter((p) => p !== fileId));
  };
  ...
  return !inboxFiles.length ? (
    <FullScreenMessage
      title={selectedInboxTabKey === 'inbox' ? 'No files in your inbox' : 'No archived files'}
      description={selectedInboxTabKey === 'inbox' ? 'Upload files to start filing' : 'Archived files will appear here'}
      ctaText="Upload"
          ...
          <div className="relative overflow-hidden">
            {selectedFiles.length > 0 && (
              <div className="absolute top-0 left-12 flex h-12 items-center space-x-3 bg-gray-50 sm:left-12">
                ...
                >
                  {`${selectedInboxTabKey === 'inbox' ? 'Archive' : 'Unarchive'} selected`}
                </button>
              </div>
            )}
            <table className="min-w-full table-fixed divide-y divide-gray-300">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {inboxFiles.map((item, index) => (
                  <tr
                    key={item.id}
                    className={`${selectedFiles.includes(item.id) ? 'bg-gray-50' : null} ${
                  ...
                  >
                    ...
                    <td
                      className={cl(
                        ...
                        selectedFiles.includes(item.id) ? 'text-indigo-600' : 'text-gray-900'
                      )}
                    >
                      {item.name}
                    </td>
