...
export default function BoardTemplate({ listId, hubName }: { listId: string; hubName: string }) {
  // eslint-disable-next-line no-console
  console.log(listId, hubName);
  // const { toggleAssignCurrentTaskId, CompactView, CompactViewWrap } = useAppSelector((state) => state.task);

  // const dispatch = useDispatch();

  // const { data: taskDataBoard } = getTaskListService({ listId });

  // const products = taskDataBoard?.data.tasks;

  // const groupBy = (key, arr) =>
  //   arr?.reduce(
  //     (cache, product) => ({
  //       ...cache,
  //       [product[key]]: product[key] in cache ? cache[product[key]].concat(product) : [product]
  //     }),
  //     {}
  //   );
  ...
  return (
    <div className="-mt-10 ">
      <p>to be fixed</p>
      {/* <p className="pb-10 my-2 font-bold uppercase " style={{ fontSize: '12px' }}>
        {hubName}
      </p>
