...
export default function Pilot() {
  ...
  const hotKeysSections = [...sections, ...DetailOptions, ...communicationOptions, ...TimeClockOptions];
    ...
    <div className="pr-0.5">
      <div
        className={`flex ease-in-out overflow-y-auto pr-1  duration-300 transition-all transform bg-white border-l border-r h-screen  ${
          !showPilot && selectedSection ? 'flex-row fixed z-40 top-16 right-0' : 'flex-col'
        }`}
        ref={hoverRef}
      ...
      >
        ...
        {selectedSection ? selectedSection.element : selectedHotKeySection ? selectedHotKeySection.element : null}
      </div>
    </div>
  );
}
