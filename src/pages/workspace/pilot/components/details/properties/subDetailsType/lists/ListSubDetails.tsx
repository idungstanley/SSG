...
export default function ListSubDetails({ listDetails }: DetailsIndexProps) {
  const [toggleSubTask, setToggleSubTask] = useState(false);
  return (
    <>
      <section className="p-2" key={listDetails?.id}>
        {/* name */}
        <div id="entity name">
        ...
        <div id="created time" className="mt-2">
          ...
          <div className="border p-1 bg-gray-100 border-white rounded-md">
            <p>{moment(listDetails?.created_at).format('MMM DD, hh:mm a')}</p>
          </div>
        </div>
        {/* due date */}
