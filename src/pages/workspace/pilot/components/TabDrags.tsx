...
export default function TabDrag({ id, name, source, showPilot, icon, showPilotIconView, subTab }: TabProps) {
  ...
  return (
    <section
      style={style}
      className={`flex flex-auto ${id === activeTabId && showPilot === false ? 'flex-col' : 'flex-row'}`}
    >
      <div
        onClick={() => handleClick(id)}
        className={cl(
          id === activeTabId ? 'bg-gray-300 text-black' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50',
      ...
      >
        {id === activeTabId && (
          <span
            className={`absolute bg-green-500 ${
              showPilotIconView ? 'top-0 left-0 right-0 h-0.5 w-fit' : 'top-0 left-0 bottom-0 w-1'
            }`}
          ></span>
        )}
        ...
        <p className={`text-xs  ${showPilot ? 'block' : 'hidden'} ${showPilotIconView ? 'hidden' : 'block'}`}>{name}</p>
      </div>
