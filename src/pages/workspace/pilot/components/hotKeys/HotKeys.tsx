...
import CustomDropdown, { IColumn } from '../../../tasks/dropdown/CustomDropdown';
...
export default function HotKeys() {
  ...
  const hotKeysTabs = [...pilotOptions, ...DetailOptions, ...communicationOptions, ...TimeClockOptions];

  const { showAddHotKeyDropdown, showRemoveHotKeyDropdown, showPilot, activeHotKeyTabId } = useAppSelector(
  ...
  useEffect(() => {
    setHotKeys(() => {
      const allVisibilityStatus: (boolean | undefined)[] | undefined = items?.map((item) => item.isVisible);
      return allHotKeysInfo.map((keys, i) => ({
        ...keys,
        isVisible: allVisibilityStatus ? allVisibilityStatus[i] : keys.isVisible
      }));
    });
  }, [items]);
  ...
  const addHotkeys = hotKeys?.filter((keys: IColumn) => keys.isVisible === false);

  const removeHotkeys = hotKeys?.filter((keys: IColumn) => keys.isVisible === true);
  ...
  return (
    <div className="">
      {removeHotkeys?.length != 0 ? (
        <div className={`border-b border-gray-200 py-2 px-4 flex gap-3 ${showPilot ? 'flex-row' : 'flex-col'}`}>
            ...
            <div key={item.index}>
              ...
              >
                <span className="flex w-4 h-4 text-sm cursor-pointer">
                  {item.icon ? item.icon : <img src={(item as IColumn).source} alt={item.name + ' icon'} />}
                </span>
              </div>
            </div>
          ))}
      ...
      {showAddHotKeyDropdown && (
        <CustomDropdown listItems={addHotkeys} title="Add HotKeys" handleClick={handleHotKey} />
      )}
      {showRemoveHotKeyDropdown && (
        <CustomDropdown listItems={removeHotkeys} title="Remove HotKeys" handleClick={handleHotKey} />
      )}
    </div>
  );
}
