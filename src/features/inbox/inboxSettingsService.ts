...
export const removeTeamMemberInboxAccessService = async (data: { inboxId?: string; teamMemberId: string }) => {
  const response = requestNew({
...
const removeTeamMemberOrGroupAccess = (data: { inboxId?: string; accessToId: string; isGroups: boolean }) => {
  const response = requestNew({
    url: `inboxes/${data.inboxId}/access/remove-access`,
    method: 'POST',
    params: {
...
export const useGetTeamMembersOrGroups = ({ query, isGroups }: { query: string; isGroups: boolean }) => {
  const queryClient = useQueryClient();
  ...
  return useInfiniteQuery<ITeamMembersAndGroupsReq>(
    ...
    },
    {
      onSuccess: (data) => {
        if (isGroups) {
          data.pages.map((page) =>
            page.data.team_member_groups?.map((item) => queryClient.setQueryData([title, item.id], item))
          );
        } else {
          data.pages.map((page) =>
            page.data.team_members?.map((item) => queryClient.setQueryData([title, item.id], item))
          );
        }
      },
