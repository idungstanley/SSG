...
export default function HotkeysList({ activeHotkeyIds, tabs, setActiveTabId, activeTabId }: HotkeysListProps) {
    ...
    <div className="flex gap-2 h-fit w-full px-2 border-t py-4 border-b">
      {hotkeys.map((hotkey) => (
        <div
          ...
          title={hotkey.label}
          className={cl(
            activeTabId === hotkey.id ? 'bg-green-500 border-green-500' : 'bg-white border-gray-200',
