...
function MenuDropdown() {
  ...
  const [shower, setIsShower] = React.useState(false);
    ...
    <Menu as="div">
      {({ open }) => (
        <Fragment>
          ...
          >
            <Menu.Items
              className="absolute w-56 py-1 origin-top-right bg-white rounded-md shadow-lg bottom-20  left-50 z-20 ring-1 ring-black ring-opacity-5 focus:outline-none"
              static
            >
              {itemsList.map((item) =>
                item.isVisible ? (
