...
function Row({ item, key }: { item: string; key: number }) {
  ...
  return (
    <tr className={`divide-gray-300 divide-y contents ${isChecked ? 'bg-gray-100' : ''}`} key={key}>
      <td className={`p-2 ${isChecked ? 'bg-alsoit-gray-75' : ''}`}>
        <div className="flex items-center justify-between gap-1">
          ...
          />
          <div
            className={`w-5 h-5 p-2 rounded ${isChecked ? 'bg-alsoit-gray-75' : ''}`}
      ...
      <td className={`p-2 text-xs truncate ${isChecked ? 'bg-alsoit-gray-75' : ''}`}>
        <div>{colorName}</div>
      </td>
    </tr>
  );
