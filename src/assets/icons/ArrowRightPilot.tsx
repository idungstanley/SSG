...
export default function ArrowRightPilot({ status, active }: ArrowRightPilotInterface) {
  return (
    <svg
      className={`transition duration-300 ${status !== 'last' && !active ? 'hover:rotate-90' : ''} ${
        active ? 'rotate-90' : ''
      }`}
      width="3.35"
      height="5.11"
    ...
    >
      <path
        ...
        fill={status == 'last' ? '#424242' : '#919191'}
      />
    </svg>
  );
}
